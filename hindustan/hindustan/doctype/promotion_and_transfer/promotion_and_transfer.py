    ...
    def on_submit(self):
        if self.promotion__transfer==1:
            if self.student_id__admission_id and not self.student_promotion:
                ...
                elif not self.custom_sem:
                    frappe.throw("Sem is required")    
                if self.promotion_academic_year and self.promotion_sem__batch and self.custom_sem:
                    ...
                    student.custom_academic_year=self.promotion_academic_year
                    student.custom_sem__batch_=self.custom_sem
                    student.save()
                    frappe.db.commit()
